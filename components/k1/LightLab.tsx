...
function createNode(id: string, templateId: string, position: { x: number; y: number }): NodeData {
  const templates: Record<string, Partial<NodeData>> = {
    gradient: {
      ...
      icon: "🌈",
      inputs: [],
      outputs: [{ id: "output", label: "Field", type: "field" }],
      parameters: [
    ...
    "hue-shift": {
      ...
      outputs: [{ id: "output", label: "Color", type: "color" }],
      parameters: [{ id: "hue", label: "Hue Shift", type: "slider", value: 0, min: -180, max: 180, step: 1 }],
    },
    blend: {
      title: "Blend",
      category: "combine",
      ...
      outputs: [{ id: "output", label: "Result", type: "color" }],
      parameters: [
...
export function LightLab() {
  ...
  const handleAddNode = (templateId: string) => {
    const newNode = createNode(`node-${Date.now()}`, templateId, { x: 300, y: 200 + nodes.length * 50 })
  ...
  const handleNodeDelete = (nodeId: string) => {
    ...
    setWires((prev) => prev.filter((wire) => wire.from.nodeId !== nodeId && wire.to.nodeId !== nodeId))
  ...
  const handleParameterChange = (nodeId: string, parameterId: string, value: number | string | boolean) => {
    setNodes((prev) =>
      prev.map((node) =>
        node.id === nodeId
          ? {
              ...node,
              parameters: node.parameters?.map((param) => (param.id === parameterId ? { ...param, value } : param)),
            }
          : node,
      ),
    )
  ...
  return (
    <div className="h-screen flex bg-[var(--k1-bg)] relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-60">
        <div
          className="absolute top-0 right-0 w-[1000px] h-[1000px] bg-gradient-to-bl from-[var(--k1-accent)]/40 via-[var(--k1-accent-2)]/30 to-transparent blur-[100px] animate-pulse"
          style={{ animationDuration: "8s" }}
        />
        <div
          className="absolute bottom-0 left-0 w-[900px] h-[900px] bg-gradient-to-tr from-[var(--k1-accent-2)]/35 via-purple-500/25 to-transparent blur-[100px] animate-pulse"
          style={{ animationDuration: "12s", animationDelay: "2s" }}
        />
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-br from-pink-500/20 via-orange-500/20 to-transparent blur-[120px] animate-pulse"
      ...
      <div className="w-80 flex-shrink-0 relative z-10">
        <NodeInspector node={selectedNode} onParameterChange={handleParameterChange} onDeleteNode={handleNodeDelete} />
      </div>
    </div>
  )
}
